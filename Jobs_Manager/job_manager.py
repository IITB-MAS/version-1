...
# ----------------------------------------------------------------------------------------- #
root = Tk()
root.title(f'{nickname}: Operator GUI')
...
def new_job_arrival():
    ...
    parameters_queue.put(message.content)
        ...
        if message.protocol == 'contract_net_interaction_protocol' and message.performative == 'call_for_proposal':
            advanced_comm_thread = threading.Thread(target=check_advanced_communications,
                                                    args=[message, timed_reply_queue, msg_to_send_queue,
                                                          paths_dictionary,parameters_queue, ])
...
listbox_receivers.grid(row=1, column=2, rowspan = 4)
clicked_agents = listbox_receivers.curselection()
all_entries.append(clicked_agents)

my_tree = ttk.Treeview(root)
# D:\MAS\Jobs_Manager\DataBase\SingleMechineSequencing\job_data_Li_Ei.csv

df = pd.read_csv(paths_dictionary['database'] + r"\SingleMechineSequencing\job_data_Li_Ei.csv")
my_tree['columns'] = ('Ji', 'pi', 'di', 'ei', 'Li', 'allotment')
my_tree.column("#0", width=120, minwidth=25)
my_tree.column("Ji", anchor=CENTER, width=80)
my_tree.column("pi", anchor=CENTER, width=80)
my_tree.column("di", anchor=CENTER, width=90)
my_tree.column("ei", anchor=CENTER, width=80)
my_tree.column("Li", anchor=CENTER, width=80)
my_tree.column("allotment", anchor=CENTER, width=150)
...
timely_queue_update_thread = threading.Thread(target=timely_queue_update, args=[])
timely_queue_update_thread.start()
