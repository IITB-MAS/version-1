...
from DownloadableFunctions import SingleMechineSequencing as SingleMachineSequencing
...
# ----------------------------------------------------------------------------------------- #
root = Tk()
root.title(f'{nickname}: manual FIPA messaging and conversations')
...
def log_allotment(Ji, Pi, Di, Ei, Li):
    ...
    message.type = 'request_queue_update'
    message.sender = nickname
    message.receiver = 'Job_Manager'
    message.content = [Ji, Pi, Di, Ei, Li,alloted_agent]
...
def mc_fail_fn():
    ...
    for job in jobs_list:
        ...
        parameters_queue.put([int(job[0]), int(job[1]), job[2], int(job[3]), int(job[4])])
        message.conversation_id = set_conversation_id(message)
        msg_to_send_queue.put(message)
        ...
        message.sender = nickname
        message.receiver = 'Job_Manager'
        message.content = [int(job[0]), int(job[1]), job[2], int(job[3]), int(job[4]), alloted_agent]
        message.conversation_id = set_conversation_id(message)
        msg_to_send_queue.put(message)
        if first_msg_flag != True:
            time.sleep(7)
        first_msg_flag = False

    # code for deleting entire queue entries
    with open(paths_dictionary['database'] + r"\SingleMechineSequencing\job_data_Li_Ei.csv", "w",
              newline='') as file:
        csv_writer = csv.writer(file)
        csv_writer.writerow(['Ji', 'Pi', 'Di', 'Ei', 'Li'])

    return

def auction_all_jobs():
    mc_fail_thread = threading.Thread(target=mc_fail_fn,
                                              args=[])
    mc_fail_thread.start()
...
def print_on_GUI_and_send_from_queue(msg_to_send_queue):
    ...
    while True:
        ...
        if message.receiver is None:
            message.receiver = targets_identifier(message.type)
        print('sending to', message.receiver)
        message.sender = nickname
        text_area.config(state='normal')
        ...
        if message.protocol == 'contract_net_interaction_protocol' and message.performative == 'call_for_proposal':
            advanced_comm_thread = threading.Thread(target = check_advanced_communications,
                                                    args= [message, timed_reply_queue, msg_to_send_queue, paths_dictionary,allotment_queue,parameters_queue, ])
